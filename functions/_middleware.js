// Cloudflare Pages Function — gates the entire site behind one shared password.
// The password lives in an Environment Variable called SITE_PASSWORD (set in
// the Cloudflare dashboard, not in this file) so you can change it anytime
...
export async function onRequest(context) {
  ...
  const isAuthed = cookieHeader
    .split(";")
    .some((c) => c.trim() === `${COOKIE_NAME}=${env.SITE_PASSWORD_HASH || "ok"}`);
